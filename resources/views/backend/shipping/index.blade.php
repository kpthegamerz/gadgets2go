...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary float-left">Shipping Charges</h6>
      ...
      <div class="table-responsive">
        ...
        <table class="table table-bordered" id="banner-dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            ...
            {{-- @php 
              $couriers=DB::table('couriers')->select('name')->where('id',$shipping->courier_id)->get();
            @endphp --}}
                <tr>
                    <td>{{$shipping->id}}</td>
                    {{-- <td>@foreach($couriers as $courier) {{$courier->name}} @endforeach</td> --}}
                    <td>{{$shipping->type}}</td>
                    <td>${{$shipping->small}}</td>
                    <td>${{$shipping->medium}}</td>
                    <td>${{$shipping->large}}</td>
                    <td>${{$shipping->xlarge}}</td>
                    <td>
